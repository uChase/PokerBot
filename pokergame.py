...
class PokerGame:
    ...
    def next_turn(self, move):
        ...
        if move == "fold":
            ...
            self.current_player["roundLoop"] = self.roundLoop
            self.current_player["amountIn"] += (
                self.roundIn - self.current_player["roundIn"]
            )
            self.pot += self.roundIn - self.current_player["roundIn"]
            self.current_player["player"].pay_amount(
                self.roundIn - self.current_player["roundIn"]
            )
            ...
            self.current_player["amountIn"] += (
                self.roundIn - self.current_player["roundIn"] + 15
            )
            self.current_player["player"].pay_amount(
                self.roundIn - self.current_player["roundIn"] + 15
            )
        ...
        if len(inPlayers) == 1:
            ...
            for player in self.players:
                if player["status"] == "out":
                    ...
                    winnerIndex = pokerutils.determine_winner(
                        clonePlayers, self.community_cards
                    )
            ...
            # WON SHOWDOWN
            inPlayers[winnerIndex]["Q"] = (
                self.pot
                / self.totalWealth
                * inPlayers[winnerIndex]["player"].get_money()
                / self.totalWealth
            )
            ...
            for player in self.players:
                if player["status"] == "out":
                    ...
                    player["Q"] = (
                        ...
                        * player["amountIn"]
                        / self.totalWealth
                        * (1 - player["player"].get_money() / self.totalWealth)
                    )
                    player["result"] = "loss"
