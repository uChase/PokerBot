...
print("There are 9 slots for players. You can choose to have a human player in one of the slots. If you do not want any more players, enter 5 in the slot. I reccomend you enter 1, 2, 2, 3, 4, 5, as a game with 5 players is easiest to follow on the terminal.")
for i in range(9):
    while True:
        userInput = input(f"Player {i+1}: Enter 1 for adapted DQN, 2 for LSTM (emulated player), 3 for raw DQN (not adapted), 4 for human player (you), 5 for no one in this slot:  ")
    ...
    if userInput == '1':
        ...
        models.append(DQN1.DQN(input_dim, output_dim).to(device))
        models[i].load_state_dict(torch.load('./models/DQN_4_2.pth'))
        models[i].eval()
    elif userInput == '4':
        models.append("human")
    else:
...
while not finished:
        ...
        if models[int(currplayer["player"].get_name())-1] == "human":
            ...
            while True:
                ...
                if userInput in valid_options:
                    ...
                    print("Invalid option. Please try again.")
            if userInput == '1':
                gameStatus, currplayer, players, pot, cardStatus, roundIn = game.next_turn("fold")
            elif userInput == '2':
                gameStatus, currplayer, players, pot, cardStatus, roundIn = game.next_turn("c")
            elif userInput == '3':
                gameStatus, currplayer, players, pot, cardStatus, roundIn = game.next_turn("raise")
            else:
                print("Invalid option. Please try again.")
        else:
    ...
    for player in playerList:
        ...
        if player.get_money() >= initialPlayerMoney[int(player.get_name())-1]:
            print(f"Player {player.get_name()} won ${player.get_money() - initialPlayerMoney[int(player.get_name())-1]}")
        else:
            print(f"Player {player.get_name()} lost ${initialPlayerMoney[int(player.get_name())-1] - player.get_money()}")
